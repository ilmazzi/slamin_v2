    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <h1 class="text-4xl font-bold text-neutral-900 dark:text-white mb-2" style="font-family: 'Crimson Pro', serif;">
                {{ __('articles.edit.title') }}
            </h1>
            <p class="text-neutral-600 dark:text-neutral-400">
                {{ __('articles.edit.subtitle') }}
            </p>
        </div>
        ...
        <form wire:submit="save" class="space-y-6">
            ...
            <div>
                <label for="excerpt" class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-2">
                    {{ __('articles.edit.excerpt_label') }}
                </label>
                <textarea id="excerpt"
                          wire:model="excerpt"
                          rows="3"
                          ...
                          placeholder="{{ __('articles.edit.excerpt_placeholder') }}"></textarea>
                @error('excerpt')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label for="category_id" class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-2">
                    {{ __('articles.edit.category_label') }}
                </label>
                <select id="category_id"
                        wire:model="category_id"
                        class="w-full px-4 py-3 border-2 border-neutral-200 dark:border-neutral-700 rounded-xl focus:border-primary-500 focus:ring-4 focus:ring-primary-100 dark:bg-neutral-800 dark:text-white transition-all">
                    <option value="">{{ __('articles.edit.no_category') }}</option>
            ...
            <div>
                <label for="tags" class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-2">
                    {{ __('articles.edit.tags_label') }}
                </label>
                <input type="text" 
                       id="tags"
                       wire:model="tags"
                       class="w-full px-4 py-3 border-2 border-neutral-200 dark:border-neutral-700 rounded-xl focus:border-primary-500 focus:ring-4 focus:ring-primary-100 dark:bg-neutral-800 dark:text-white transition-all"
                       placeholder="{{ __('articles.edit.tags_placeholder') }}">
                <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
                    {{ __('articles.edit.tags_hint') }}
                </p>
                @error('tags')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div>
                <label class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-2">
                    {{ __('articles.edit.featured_image_label') }}
                </label>
                ...
                @if($existing_featured_image)
                    <div class="mb-4">
                        <img src="{{ $existing_featured_image }}" 
                             ...
                             class="w-full max-w-md h-48 object-cover rounded-xl border-2 border-neutral-200 dark:border-neutral-700">
                    </div>
                @endif
                ...
                @if($featured_image)
                    <p class="mt-2 text-sm text-neutral-600 dark:text-neutral-400">
                        {{ __('articles.edit.image_uploaded') }}: {{ $featured_image->getClientOriginalName() }}
                    </p>
                @endif
            </div>
            ...
            <div>
                <label class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-2">
                    {{ __('articles.edit.content_label') }} *
                </label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="status" class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-2">
                        {{ __('articles.edit.status_label') }} *
                    </label>
                    <select id="status"
                            wire:model="status"
                            class="w-full px-4 py-3 border-2 border-neutral-200 dark:border-neutral-700 rounded-xl focus:border-primary-500 focus:ring-4 focus:ring-primary-100 dark:bg-neutral-800 dark:text-white transition-all">
                        <option value="draft">{{ __('articles.edit.status_draft') }}</option>
                        <option value="published">{{ __('articles.edit.status_published') }}</option>
                        <option value="archived">{{ __('articles.edit.status_archived') }}</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    <label for="published_at" class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-2">
                        {{ __('articles.edit.published_at_label') }}
                    </label>
            ...
            <div class="flex items-center gap-3">
                ...
                <label for="is_public" class="text-sm font-semibold text-neutral-700 dark:text-neutral-300">
                    {{ __('articles.edit.is_public_label') }}
                </label>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        console.log('✅ Quill initialized for article editing');
    });
</script>
@endpush

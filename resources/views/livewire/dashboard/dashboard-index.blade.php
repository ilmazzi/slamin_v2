        ...
        <div x-data="{ visible: false }" 
            ...
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8"
                ...
                <div class="flex-1">
                    <h2 class="text-3xl md:text-4xl font-bold text-neutral-900 dark:text-white mb-2"
                        style="font-family: 'Crimson Pro', serif;">
                        📅 {{ __('dashboard.my_calendar') }}
                    </h2>
                    <p class="text-neutral-600 dark:text-neutral-400">{{ __('dashboard.calendar_subtitle') }}</p>
                </div>
            ...
            <div x-data="{ calendarVisible: false }" 
                ...
                <div class="flex md:hidden gap-2 w-full md:w-auto order-first md:order-none mb-4 md:mb-0">
                    ...
                    <button 
                        ...
                        class="flex-1 px-3 py-2.5 rounded-xl {{ $currentView === 'week' ? 'bg-primary-600 text-white shadow-lg shadow-primary-600/30' : 'bg-white dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 border border-neutral-200 dark:border-neutral-700' }} font-medium transition-all duration-300 flex items-center justify-center gap-1.5">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                        </svg>
                    ...
                    <button 
                        ...
                        class="flex-1 px-3 py-2.5 rounded-xl {{ $currentView === 'month' ? 'bg-primary-600 text-white shadow-lg shadow-primary-600/30' : 'bg-white dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 border border-neutral-200 dark:border-neutral-700' }} font-medium transition-all duration-300 flex items-center justify-center gap-1.5">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex items-center gap-3 {{ $currentView !== 'month' ? 'hidden md:flex' : 'flex' }}">
                    ...
                    <div class="px-6 py-2 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl min-w-[180px] text-center">
                        <span class="font-semibold text-neutral-900 dark:text-white">{{ $calendarData['monthName'] }}</span>
                    </div>
            ...
            <div class="md:hidden mb-6" x-data="{ visible: false }" x-init="setTimeout(() => visible = true, 100)">
                ...
                <div class="space-y-6"
                     ...
                     x-transition:enter-start="opacity-0 translate-y-4"
                     x-transition:enter-end="opacity-100 translate-y-0">
                    @forelse($paginatedEvents as $date => $events)
                        <!-- Date Header -->
                        <div class="relative">
                            <div class="flex items-center gap-3 mb-4">
                                <div class="flex-shrink-0 w-16 h-16 rounded-2xl bg-primary-600 dark:bg-primary-700 flex flex-col items-center justify-center text-white shadow-lg">
                                    <span class="text-2xl font-bold leading-none">{{ \Carbon\Carbon::parse($date)->format('d') }}</span>
                                    <span class="text-xs uppercase">{{ \Carbon\Carbon::parse($date)->isoFormat('MMM') }}</span>
                                </div>
                                <div class="flex-1">
            ...
            <div class="md:hidden mb-6" x-data="{ visible: false }" x-init="setTimeout(() => visible = true, 100)">
                ...
                <div class="space-y-4"
                    ...
                    @for($i = 0; $i < 7; $i++)
                        ...
                        <!-- Day Container -->
                        <div class="overflow-hidden rounded-2xl {{ $day->isToday() ? 'ring-2 ring-primary-500 shadow-xl shadow-primary-500/20' : '' }}">
                            <!-- Day Header -->
            ...
            <div class="{{ $currentView === 'month' ? 'block' : 'hidden md:block' }} bg-white dark:bg-neutral-900 rounded-3xl shadow-xl overflow-hidden"
                 wire:key="calendar-{{ $currentMonth }}-{{ $currentYear }}"
                 x-show="visible"
                        ...
                        <div class="grid grid-cols-7 gap-3 mb-3">
                            @foreach($week as $dayIndex => $day)
                                <div class="relative rounded-2xl transition-all duration-300 overflow-hidden group cursor-pointer
                                            {{ $day['isCurrentMonth'] ? 'bg-white dark:bg-neutral-800 hover:shadow-xl hover:scale-105 hover:-translate-y-1' : 'opacity-40 bg-neutral-50 dark:bg-neutral-900' }}
                                            {{ $day['isToday'] ? 'ring-2 ring-primary-500 shadow-xl shadow-primary-500/30' : 'shadow-md' }}"
                                     style="height: 140px;">
                                    
                                    <!-- Numero giorno - PULITO senza sfondo -->
                                    <div class="absolute top-2 left-2 z-30">
                                        <span class="font-bold text-base drop-shadow-lg
                                                    {{ $day['isToday'] ? 'text-primary-600' : 'text-neutral-900 dark:text-white group-hover:text-primary-600 transition-colors' }}"
                                              style="{{ $day['isToday'] ? 'text-shadow: 0 0 15px rgba(5, 150, 105, 1), 0 0 8px rgba(5, 150, 105, 0.8), 0 2px 4px rgba(0,0,0,0.3);' : 'text-shadow: 0 2px 4px rgba(0,0,0,0.15);' }}">
                                            {{ $day['date']->day }}
                                        </span>
                                    </div>
                                                    ...
                                                    <div class="absolute inset-0 flex flex-col justify-end p-3">
                                                        ...
                                                        @if($day['events']->count() > 1)
                                                            <div class="flex justify-center gap-1.5 mt-2">
                                                                @foreach($day['events'] as $idx => $e)
                                                                    <div class="h-1 rounded-full transition-all duration-300"
                                                                         :class="currentSlide === {{ $idx }} ? '{{ $event['image'] ?? false ? 'bg-white' : 'bg-primary-600' }} w-6' : '{{ $event['image'] ?? false ? 'bg-white/40' : 'bg-neutral-300' }} w-1.5'">
                                                                    </div>
                                                                @endforeach
                                                            </div>
                                                        @endif
            ...
            <div class="flex flex-wrap items-center justify-center gap-6 mt-6"
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm text-neutral-600 dark:text-neutral-400">{{ __('dashboard.today') }}</span>
                </div>
            </div>
                ...
                <div class="relative overflow-hidden" 
                    ...
                    <div class="mb-8"
                        ...
                        <h2 class="text-2xl md:text-3xl font-bold text-neutral-900 dark:text-white mb-2"
                            style="font-family: 'Crimson Pro', serif;">
                            {{ __('dashboard.social_activities') }}
                        </h2>
                        <p class="text-neutral-600 dark:text-neutral-400">
                            {{ __('dashboard.social_activities_subtitle') }}
                        </p>
                    </div>
                       ...
                       x-transition:enter-end="opacity-100 scale-100">
                        ...
                        <div class="relative h-full flex flex-col justify-between">
                            <div>
                                ...
                                <h3 class="text-2xl font-bold text-white mb-2">{{ $socialActivities[0]['title'] }}</h3>
                                <p class="text-white/80 text-sm">{{ $socialActivities[0]['description'] }}</p>
                            </div>
                            
                            <!-- Numero grande e elegante -->
                            <div class="text-right">
                                <span class="text-7xl font-black text-white/20 group-hover:text-white/30 transition-colors duration-500">{{ $socialActivities[0]['count'] }}</span>
                            </div>
                        </div>
                    </a>

                    <!-- GRUPPI - Medio, 1 colonna -->
                    <a href="{{ $socialActivities[1]['url'] }}"
                        ...
                        <div class="relative h-full flex flex-col justify-between">
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-2">
                                    <span class="text-3xl font-black text-white">{{ $socialActivities[2]['count'] }}</span>
                                </div>
                                <h3 class="text-sm font-bold text-white/90">{{ $socialActivities[2]['title'] }}</h3>
                            </div>
                        </div>
                    </a>

                    <!-- MESSAGGI - Grande orizzontale, 2 colonne -->
                    <a href="{{ $socialActivities[3]['url'] }}"
                       ...
                       x-transition:enter-end="opacity-100 scale-100">
                        ...
                        <div class="relative flex items-center justify-between h-full">
                            ...
                            <div class="text-right">
                                <span class="text-4xl font-black text-white">{{ $socialActivities[3]['count'] }}</span>
                            </div>
                        </div>
                    </a>
                </div>
        ...
        <div class="py-12 md:py-20">
                                                ...
                                                <div class="space-y-2 text-sm text-neutral-600 dark:text-neutral-400">
                                                    ...
                                                    @if($event['venue'] || $event['city'])
                                                        <div class="flex items-center gap-2">
                                                            ...
                                                            <span>{{ $event['venue'] }}@if($event['city']), {{ $event['city'] }}@endif</span>
                                                        </div>
                                                    @endif
                                                </div>
                                            </div>
            ...
            @else
                ...
                <div class="text-center py-16"
                    ...
                    <h3 class="text-lg font-bold text-neutral-900 dark:text-white mb-2">{{ __('dashboard.no_recent_activity') }}</h3>
                </div>
            @endif
                </div>
            </div>
        ...
        <div class="py-12 md:py-20">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-8"
                    ...
                    <h2 class="text-2xl md:text-3xl font-bold text-neutral-900 dark:text-white mb-2"
                        style="font-family: 'Crimson Pro', serif;">
                        {{ __('dashboard.recent_activity_title') }}
                    </h2>
                    <p class="text-neutral-600 dark:text-neutral-400">
                        {{ __('dashboard.recent_activity_subtitle') }}
                    </p>
                </div>
                ...
                <div class="flex gap-6 overflow-x-auto pb-4 scrollbar-hide">
                    ...
                    <div x-show="visible"
                        ...
                        <div class="relative">
                            ...
                            <!-- Content -->
                            <h3 class="font-bold text-neutral-900 dark:text-white mb-2">{{ $activity['title'] }}</h3>
                            <p class="text-sm text-neutral-600 dark:text-neutral-400 mb-3">{{ $activity['description'] }}</p>

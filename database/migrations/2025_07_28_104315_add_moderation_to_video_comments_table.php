...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('video_comments', function (Blueprint $table) {
            if (!Schema::hasColumn('video_comments', 'moderation_status')) {
                $table->enum('moderation_status', ['pending', 'approved', 'rejected'])->default('pending')->after('report_count');
            }
            if (!Schema::hasColumn('video_comments', 'moderation_notes')) {
        ...
        Schema::table('video_comments', function (Blueprint $table) {
            if (!Schema::hasIndex('video_comments', 'video_comments_moderation_status_index')) {
                $table->index('moderation_status');
            }
            if (!Schema::hasIndex('video_comments', 'video_comments_moderated_by_index')) {

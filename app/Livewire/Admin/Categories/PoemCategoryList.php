...
class PoemCategoryList extends Component
{
    use WithPagination;
    ...
    protected function rules()
    {
        $rules = [
            ...
            'slug' => 'required|string|max:255|unique:poem_categories,slug',
        ...
        if ($this->editingId) {
            $rules['slug'] = 'required|string|max:255|unique:poem_categories,slug,' . $this->editingId;
        }
    ...
    {
        $category = PoemCategory::findOrFail($id);
    ...
    public function save()
    {
        ...
        if ($this->editingId) {
            $category = PoemCategory::findOrFail($this->editingId);
            $category->update($data);
            session()->flash('message', 'Categoria aggiornata con successo!');
        } else {
            PoemCategory::create($data);
    ...
    public function delete($id)
    {
        ...
        if ($category->poems()->count() > 0) {
            session()->flash('error', 'Impossibile eliminare: ci sono poesie associate a questa categoria.');
            return;
        }
    ...
    public function render()
    {
        ...
        return view('livewire.admin.categories.poem-category-list', [
            'categories' => $categories
        ])->layout('components.layouts.app')->title('Gestione Categorie Poesie');
    }
}
